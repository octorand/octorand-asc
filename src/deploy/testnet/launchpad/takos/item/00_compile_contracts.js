    ...
    try {
        ...
        for (let i = 0; i < contracts.length; i++) {
            ...
            if (!application['application_id']) {
                ...
                console.log('launchpad takos item ' + contract + ' approval program length is ' + (await testnet.compile(approvalProgram)).length + ' bytes');
                console.log('launchpad takos item ' + contract + ' clear program length is ' + (await testnet.compile(clearProgram)).length + ' bytes');
            }
        }
    } catch (error) {
        console.log(error);
