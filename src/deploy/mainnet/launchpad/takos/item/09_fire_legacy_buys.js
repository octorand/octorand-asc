...
exports.execute = async function () {
    ...
    let events = JSON.parse(fs.readFileSync('src/deploy/mainnet/launchpad/takos/item/events.json'));

    let buyContract = new connection.baseClient.ABIContract(JSON.parse(fs.readFileSync('src/build/mainnet/launchpad/takos/item/buy/contract.json')));
    ...
    for (let i = 0; i < max; i++) {
        ...
        if (!items[i]['fired_buys']) {
            ...
            let profile = events['items'].find(x => x.id == 'takos-' + String(items[i].id).padStart(4, '0'));
            if (profile) {
