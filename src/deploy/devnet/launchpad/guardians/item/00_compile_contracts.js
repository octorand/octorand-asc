    ...
    try {
        ...
        for (let i = 0; i < contracts.length; i++) {
            ...
            if (!application['application_id']) {
                ...
                console.log('launchpad guardians item ' + contract + ' approval program length is ' + (await devnet.compile(approvalProgram)).length + ' bytes');
                console.log('launchpad guardians item ' + contract + ' clear program length is ' + (await devnet.compile(clearProgram)).length + ' bytes');
            }
        }
    } catch (error) {
        console.log(error);
