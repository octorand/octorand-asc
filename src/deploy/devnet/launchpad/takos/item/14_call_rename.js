...
exports.execute = async function () {
    try {
        ...
        let contract = new connection.baseClient.ABIContract(JSON.parse(fs.readFileSync('src/build/devnet/launchpad/takos/item/rename/contract.json')));

        let application = config['launchpad']['takos']['contracts']['item']['rename'];
        ...
        if (!application['renamed']) {
            ...
            composer.addTransaction({
                ...
                txn: connection.baseClient.makeAssetTransferTxnWithSuggestedParamsFromObject({
                    from: sender,
                    to: connection.takos.treasury.addr,
                    assetIndex: config['setup']['takos']['asset_id'],
            ...
            config['launchpad']['takos']['contracts']['item']['rename'] = application;
