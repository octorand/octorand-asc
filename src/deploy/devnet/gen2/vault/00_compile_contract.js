...
exports.execute = async function () {
    try {
        ...
        if (!vault['application_id']) {
            ...
            console.log('gen2 vault approval program length is ' + (await devnet.compile(approvalProgram)).length + ' bytes');
            console.log('gen2 vault clear program length is ' + (await devnet.compile(clearProgram)).length + ' bytes');
        }
